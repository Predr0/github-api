    ...
    <main className="min-h-screen bg-slate-950 text-slate-50 p-4 md:p-8">
      <div className="max-w-4xl mx-auto space-y-12">
        ...
        <section className="text-center space-y-4 pt-8">
          ...
          <p className="text-slate-400 text-lg">Insira um usuário do GitHub para analisar perfil e repositórios.</p>
          <div className="pt-4">
            <SearchInput />
          </div>
        </section>
        ...
        {username ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 border-2 border-dashed border-slate-900 rounded-3xl bg-slate-900/20">
            ...
            <p className="text-slate-600 text-sm">Tente pesquisar pelo seu próprio usuário!</p>
          </div>
        )}

        <footer className="text-center text-slate-600 text-xs pt-12">
          <p>Desenvolvido com Next.js 15 & Tailwind CSS</p>
        </footer>
      </div>
    </main>
  );
        ...
        <section className="flex flex-col md:flex-row items-center gap-8 p-8 bg-slate-900 rounded-2xl border border-slate-800 shadow-xl">
          ...
          <div className="text-center md:text-left flex-1 space-y-4">
            ...
            <p className="text-slate-300 leading-relaxed max-w-xl">
              {user.bio || "Este usuário preferiu manter o mistério e não adicionou uma bio."}
            </p>
            
            <div className="flex flex-wrap gap-4 justify-center md:justify-start">
              <div className="flex items-center gap-2 bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-700">
                <User size={16} className="text-blue-400"/>
                <span className="text-sm font-medium">{user.followers} <span className="text-slate-500">seguidores</span></span>
              </div>
              <div className="flex items-center gap-2 bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-700">
                <Folder size={16} className="text-purple-400"/>
                <span className="text-sm font-medium">{user.public_repos} <span className="text-slate-500">repositórios</span></span>
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {repos.map((repo) => (
              <a 
                ...
                className="group p-5 bg-slate-900 rounded-xl border border-slate-800 hover:border-blue-500/50 hover:bg-slate-800/50 transition-all duration-300"
              >
                ...
                <p className="text-sm text-slate-400 mt-3 line-clamp-2 min-h-[40px]">
                  {repo.description || "Nenhuma descrição fornecida para este projeto."}
                </p>
                
                <div className="mt-5 flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.5)]" />
                  <span className="text-xs font-mono text-slate-300">{repo.language || "Markdown"}</span>
                </div>
              </a>
            ))}
          </div>
    ...
    return (
      <div className="flex flex-col items-center gap-4 p-12 bg-red-950/10 border border-red-900/30 rounded-2xl text-center">
        ...
        <div className="space-y-1">
          <h3 className="text-lg font-bold text-red-500">Usuário não encontrado</h3>
          <p className="text-slate-400">Verifique se o @username está correto e tente novamente.</p>
        </div>
      </div>
    );
  }
